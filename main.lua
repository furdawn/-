...
local function meowfag()
    ...
    for _, v in pairs(RealCharacter:GetChildren()) do
        if v:IsA("LocalScript") then
            local clone = v:Clone()
            clone.Disabled = true
            clone.Parent = InvisCharacter
        end
    end
    for _, v in pairs(InvisCharacter:GetDescendants()) do
    ...
    local function RealCharacterDied()
        ...
        RealCharacter = Players.LocalPlayer.Character
        IsInvisible = false
        InvisCharacter:Destroy()
    ...
    Players.LocalPlayer.CharacterAppearanceLoaded:Connect(RealCharacterDied)

    local PseudoAnchor
    game:GetService "RunService".RenderStepped:Connect(
        function()
            if PseudoAnchor ~= nil then
                PseudoAnchor.CFrame = inLobby.CFrame * CFrame.new(0, 5, 0)
            end
        end
    )
    PseudoAnchor = InvisCharacter.HumanoidRootPart

    local function Invisible()
        StoredCF = RealCharacter.HumanoidRootPart.CFrame
        RealCharacter.HumanoidRootPart.CFrame = InvisCharacter.HumanoidRootPart.CFrame
        InvisCharacter.HumanoidRootPart.CFrame = StoredCF
        RealCharacter.Humanoid:UnequipTools()
        Players.LocalPlayer.Character = InvisCharacter
        workspace.CurrentCamera.CameraSubject = InvisCharacter.Humanoid
        PseudoAnchor = RealCharacter.HumanoidRootPart
        for _, v in pairs(InvisCharacter:GetChildren()) do
            if v:IsA("LocalScript") then
                v.Disabled = false
            end
        end
    ...
    local function unInvisible()
        StoredCF = InvisCharacter.HumanoidRootPart.CFrame
        InvisCharacter.HumanoidRootPart.CFrame = RealCharacter.HumanoidRootPart.CFrame
        RealCharacter.HumanoidRootPart.CFrame = StoredCF
        InvisCharacter.Humanoid:UnequipTools()
        Players.LocalPlayer.Character = RealCharacter
        workspace.CurrentCamera.CameraSubject = RealCharacter.Humanoid
        PseudoAnchor = InvisCharacter.HumanoidRootPart
        for i, v in pairs(InvisCharacter:GetChildren()) do
            if v:IsA("LocalScript") then
                v.Disabled = true
            end
        end
